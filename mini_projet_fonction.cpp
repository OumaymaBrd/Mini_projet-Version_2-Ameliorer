...
void fonction_affichage() {
    printf("\n\t\t\033[0mLe programme offre les fonctionnalités suivantes :\n\n");
...
int nb_livre() {
    ...
    while (1) {
        ...
        // Vérifie si tous les caractères sont numériques
        int valid = 1;
        for (int j = 0; buffer[j] != '\0'; j++) {
            if (!isdigit(buffer[j]) && buffer[j] != '\n') {
                valid = 0; // Si un caractère n'est pas un chiffre, on marque comme invalide
                break;
            }
        }
...
int verifier_nombre(char str[100]) {
    for (int j = 0; str[j] != '\0'; j++) {
        if (!isdigit(str[j]) && str[j] != '\n') {
            return 0; // Si un caractère n'est pas un chiffre, on marque comme invalide
        }
    }
...
void affichage_tableau(int nb, char titre[][500], char auteur[][500], float prix[], int quantite[]) {
  if (nb == 0) {
    printf("\n\t\t\033[1;31m Ooops!!!!! Aucun livre n'a été ajouté au stock.\n\n");
} else {
    // Affichage du tableau seulement si nb est supérieur à 0
    printf("\n\n\t***** Affichage de Tous les Livres Disponibles *****\n\n");
    printf("\t%-15s %-15s %-10s %-10s\n", "Titre", "Auteur", "Prix", "Quantité");
    printf("\t-----------------------------------------------------\n");

    for (int i = 0; i < nb; i++) {
        if (titre[i][0] != '\0') { // Vérifie si le titre n'est pas marqué comme supprimé
...
int verifier_saisir_nombre() {
    ...
    while (1) {
        ...
        for (int j = 0; buffer[j] != '\0'; j++) {
            if (!isdigit(buffer[j])) {
                valid = 0; // Trouvé un caractère non numérique
                break;
            }
        }
...
void mettre_a_jour_quantite(int nb, char titre[][500], int quantite[]) {
    ...
    printf("\n\n\t\tSaisir le titre du livre à rechercher : ");
        ...
        if (strcmp(titre[i], livre) == 0) {
            ...
            while (1) {
                ...
                if (verifier_nombre(buffer) && sscanf(buffer, "%d", &nv) == 1 && nv >= 0) {
                    quantite[i] = nv; // Met à jour la quantité
                    printf("\n\n\t\t ** \033[1;33mQuantité mise à jour avec succès ! \033[1;0m**\n\n");
                    return;
                } else {
                    printf("\n\n\t\033[1,31mErreur : Veuillez entrer une quantité valide (nombre entier positif).\033[1;0m\n");
                    printf("\t\033[1;35mReSaisir la nouvelle quantité : \033[1;0m");
                }
            }
        }
    }

    printf("\t\033[1;31m !!! Aucun livre trouvé avec ce titre.\n");
}
void supprimer_livre(int nb, char titre[][500], char auteur[][500], float prix[], int quantite[]) {
    ...
    int found = 0; // Pour vérifier si le livre a été trouvé

    for (int i = 0; i < nb; i++) {
        if (strcmp(titre[i], livre) == 0 && titre[i][0] != '\0') { // Vérifie si le livre est dans la liste
            titre[i][0] = '\0'; // Marque le livre comme supprimé
            auteur[i][0] = '\0'; // Vider l'auteur
            prix[i] = 0; // Vider le prix
            quantite[i] = 0; // Vider la quantité
            found = 1; // Livre trouvé
    ...
    if (!found) {
        printf("\033[1;31mLivre non trouvé.\033[0m\n"); // Affiche une erreur si le livre n'est pas trouvé
    }
}
...
void afficher_nombre_total(int nb, int quantite[]) {
    int total = 0;
    for (int i = 0; i < nb; i++) {
        total += quantite[i]; // Additionne toutes les quantités
    }
    ...
    do{
        ...
        case 5:{
           ...
           for (int i = 0; i < nb; i++) {
            ...
            afficher_nombre_total(nb, quantite); // Affiche le total après ajout
    break;
